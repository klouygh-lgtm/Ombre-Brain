...
def _is_sealed(bucket: dict) -> bool:
    """判断桶是否被封存。"""
    return bool(bucket.get("metadata", {}).get("sealed") or bucket.get("sealed"))
...
snapshot_store = SnapshotStore(config.get("state_dir", "/state"))  # Snapshot store / 写前快照

# --- Seal: 暗语验真 ---
OMBRE_SEAL = os.environ.get("OMBRE_SEAL", "").strip()
if not OMBRE_SEAL:
    logger.warning("⚠️ OMBRE_SEAL 环境变量未设置，记忆验真功能已禁用。建议在 docker-compose.yml 中设置。")
...
3. anchor 应该是未来长期都会反复帮助理解用户、关系、承诺、重要经历或长期项目的记忆。
...
async def _collect_diffusion_seed_buckets(query: str, max_seeds: int) -> tuple[list[dict], list[str]]:
    ...
    try:
        ...
        logger.warning(f"Inspect diffusion keyword search failed / 扩��诊��关键词检索失败: {e}")
...
async def breath(
    ...
    if OMBRE_SEAL:
        import hashlib
        seal_hash = hashlib.sha256(f"{OMBRE_SEAL}:breath:{len(returned_moments)}".encode()).hexdigest()[:12]
        response_text += f"\n[seal:{seal_hash}]"
    return response_text
...
async def read_bucket(bucket_id: str) -> dict:
    """按 bucket_id 精确读取完整记忆桶；trace/comment 前先读。只读，不刷新活��度。"""
...
# =============================================================
# --- 矛盾检测辅助函数 ---
async def _detect_conflict(new_content: str, new_bucket_id: str) -> str:
    """检索语义最近的旧桶，对比日期和数字，发现冲突返回警告字符串。"""
    import re as _re
    try:
        similar = await embedding_engine.search(new_content, top_k=3)
    except Exception:
        return ""
    # 提取数字和日期
    def _extract_facts(text: str) -> dict:
        dates = set(_re.findall(r'\d{4}[-/]\d{1,2}[-/]\d{1,2}', text))
        numbers = set(_re.findall(r'\b\d+(?:\.\d+)?\b', text))
        return {"dates": dates, "numbers": numbers}

    new_facts = _extract_facts(new_content)
    if not new_facts["dates"] and not new_facts["numbers"]:
        return ""
    ...
    for item in similar:
        ...
        if not bucket:
            continue
        old_text = bucket.get("content", "")
        old_facts = _extract_facts(old_text)
        # 日期冲突：同一语义但日期不同
        if new_facts["dates"] and old_facts["dates"] and new_facts["dates"] != old_facts["dates"]:
            conflicts.append(f"⚠️ 与 [{bid}] 日期可能冲突: 新={new_facts['dates']} vs 旧={old_facts['dates']}")
        # 数字冲突：语义相似但关键数字不同（只在有少量数字时检测）
        if len(new_facts["numbers"]) <= 5 and len(old_facts["numbers"]) <= 5:
            diff = new_facts["numbers"].symmetric_difference(old_facts["numbers"])
            if diff and new_facts["numbers"] & old_facts["numbers"]:  # 有交集但也有差异
                pass  # 数字差异太常见，暂不报
        if len(conflicts) >= 2:
            break
    if conflicts:
...
async def hold(
    ...
    conflict_warning = ""
    try:
        conflict_warning = await _detect_conflict(content, bucket_id)
    except Exception as e:
        logger.debug(f"矛盾检测跳过: {e}")

    return f"{action}{result_name} {','.join(domain)}{related_note}{conflict_warning}"
...
async def darkroom_enter(
    ...
    """写入一段未显影的私密反思；默认第一人称，不用第三人称自述；默认新开房间，new_room=false 才续写当前 active 房间；写后要撤回已有房间时传 new_room=false + visibility="retracted"；不回显 note 正文。"""
    try:
...
async def grow(content: str, auto: bool = False, source: str = "", title: str = "", context: Context | None = None) -> str:
    ...
    if len(content.strip()) < 30:
        ...
        try:
            ...
            logger.warning(f"Fast-path analyze failed / 快速路径指标失败: {e}")
            analysis = {
                "domain": ["general"], "valence": 0.5, "arousal": 0.3,
                "tags": [], "suggested_name": "",
            }
...
async def trace(
    ...
    """修改已有记忆，不创建新桶。tags/domain/content 默认是替换；append=True 时 content 追加到原内容末尾。sealed=1 封存桶（从所有浮现/检索中隐藏，与 pinned 互斥）；sealed=0 解封。date 可改事件日期；改前先 read_bucket。resolved/digested 让旧事沉底。只改元数据/date 不重建 embedding，改 content/name 才重建。"""
    ...
    if sealed in (0, 1):
        if sealed == 1 and (bucket.get("metadata", {}).get("pinned") or bucket.get("pinned")):
            return "封存与钉选互斥，请先取消 pinned 再封存。"
        updates["sealed"] = bool(sealed)
    if content:
        # --- 写前快照：保存旧 content ---
        old_content = bucket.get("content", "")
        if old_content:
            snapshot_store.save(bucket_id, old_content, operation="append" if append else "replace")
        # --- 追加模式 ---
        if append:
            updates["content"] = (old_content + "\n" + content).strip()
        else:
...
async def snapshots(
    ...
    """查看某桶的写前快照历史。传 bucket_id 列出版本列表；额外传 snapshot_id 可读取某个快照的完整内容。"""
    bucket_id = _coerce_memory_id(bucket_id)
    if not bucket_id:
        return "请提供有效的 bucket_id。"

    if snapshot_id >= 0:
        version = snapshot_store.get_version(snapshot_id)
        if not version:
            return f"快照 #{snapshot_id} 不存在。"
        return (
            f"=== 快照 #{version['id']} ===\n"
            f"桶: {version['bucket_id']}\n"
            f"操作: {version['operation']}\n"
            f"时间: {version['timestamp']}\n\n"
            f"{version['content']}"
        )

    versions = snapshot_store.list_versions(bucket_id, limit=limit)
    if not versions:
        return f"桶 {bucket_id} 暂无历史快照。"

    lines = [f"=== 桶 {bucket_id} 的写前快照（最近 {len(versions)} 条）==="]
    for v in versions:
        lines.append(f"  #{v['id']}  {v['operation']}  {v['timestamp']}")
    return "\n".join(lines)
...
# =============================================================
@mcp.tool()
async def pulse(include_archive: bool = False, include_sealed: bool = False, limit: int = 15) -> str:
    """只读查看系统状态和记忆桶摘要；用于盘点和找 read_bucket/trace 候选。limit 控制普通桶最大显示数（pinned/protected 不计入）��include_sealed=True 可显示被封存的桶。"""
    try:
        stats = await bucket_mgr.get_stats()
    except Exception as e:
        return f"获取系统状态失败: {e}"
    ...
    try:
        buckets = await bucket_mgr.list_all(include_archive=include_archive, include_sealed=include_sealed)
    except Exception as e:
        return status + f"\n列出记忆桶失败: {e}"
    ...
    if OMBRE_SEAL:
        import hashlib
        seal_hash = hashlib.sha256(f"{OMBRE_SEAL}:pulse:{len(lines)}".encode()).hexdigest()[:12]
        result += f"\n[seal:{seal_hash}]"
    return result
...
def _clean_profile_object(value: str) -> str:
    ...
    text = re.sub(r"(这件事|这个设定|这个东西|的时候)$", "", text)
...
async def reflect(period: str = "daily", force: bool = False) -> dict:
    """生成 daily relationship_weather 类型的 feel,记录当天关系天气,��文会带 affect_anchor 和弦。weekly 默认关闭,需 reflection.weekly_enabled=true 才会生成; force=True 会重写同周期结果。它不会替代 hold/grow 写具体 bucket。"""
    await decay_engine.ensure_started()
...
if __name__ == "__main__":
    ...
    if transport in ("sse", "streamable-http"):
        ...
        if _bool_value(word_map_cfg.get("daily_rebuild_enabled"), True):
            ...
            wt.start()
            logger.info("Word Map daily rebuild scheduler started / 词图每日��建定时器已启动")
        ...
        dt.start()
        logger.info("Dream scheduler loop started / 夜��定时��循环已启动")
